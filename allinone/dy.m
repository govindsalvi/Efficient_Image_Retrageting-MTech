clear all
I = load_images('dy/ImageDataICCP09/ArchSequence');
...
for i=1:window:(sz(1)-window)
     for j=1:window:(sz(2)-window)
         L=checkdy(Igray(i:(i+window-1),j:(j+window-1),1),Igray(i:(i+window-1),j:(j+window-1),2),V(i:(i+window-1),j:(j+window-1)),maxv);
         if L==0
             s=s+1;
         else
             I2(i:(i+window-1),j:(j+window-1),:)=mark_dy_region(I1(i:(i+window-1),j:(j+window-1),:),I2(i:(i+window-1),j:(j+window-1),:),window);
             d=d+1;
         end
     end
 end
