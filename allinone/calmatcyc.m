...
for j=2:sz(2)
    for i=1:sz(1)
        ...
        elseif i==sz(1)
            ...
            cb(i,j)=gb(i,j)+min([cb(i-1,j-1),cb(i,j-1)]);
            cc(i,j)=gc(i,j)+min([cc(i-1,j-1),cc(i,j-1),]);
           
        else
            ca(i,j)=ga(i,j)+min([ca(i-1,j-1),ca(i,j-1),ca(i+1,j-1)]);
            cb(i,j)=gb(i,j)+min([cb(i-1,j-1),cb(i,j-1),cb(i+1,j-1)]);
            cc(i,j)=gc(i,j)+min([cc(i-1,j-1),cc(i,j-1),cc(i+1,j-1)]);
...
row=adjust(minra,ca,minrb,cb,minrc,cc);
